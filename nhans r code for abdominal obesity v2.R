...
htn.logit.model3 <- svyglm(htn.cat~factor(race)*factor(abd_obesity)+ factor(sex) +factor(age.cat), design = nhanes.ao.design, family='binomial')
summary(htn.logit.model3)
round(exp(coef(htn.logit.model3)),2)

htn.logit.model2 <- svyglm(htn.cat~abd_obesity*factor(age.cat) + factor(race) +factor(sex), design = nhanes.ao.design, family='binomial')

htn.logit.model <- svyglm(htn.cat~abd_obesity*factor(age.cat) + abd_obesity*factor(race) +abd_obesity*factor(sex), design = nhanes.ao.design, family='binomial')
summary(htn.logit.model)
...
#DM
dm.logit.model <- svyglm(dm.cat~abd_obesity*age.cat + abd_obesity*race +abd_obesity*sex, design = nhanes.ao.design, family='binomial')
summary(dm.logit.model)
#get the OR
round(exp(coef(dm.logit.model)),2)

#TG
hdl.logit.model <- svyglm(low.hdl~abd_obesity*age.cat + abd_obesity*race +abd_obesity*sex, design = nhanes.ao.design, family='binomial')
summary(hdl.logit.model)
#get the OR
round(exp(coef(hdl.logit.model)),2)


#ggplot 
colnames(ao.nhanes)
tab.codm.w = svytable(~abd_obesity+dm.cat, design=nhanes.ao.design)
tab.codm.w
dataframe.codm.w <- tab.codm.w %>% 
  as.data.frame() %>% 
  group_by(abd_obesity) %>% 
  mutate(prop.dm = Freq/sum(Freq))
dataframe.codm.w

tab.codm.w %>% 
  as.data.frame() %>% 
  ggplot() +
  aes(x=abd_obesity, fill=dm.cat, y = Freq)+
  geom_col()
