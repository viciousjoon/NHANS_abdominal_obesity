...
#for table 1
svytable(~abd_obesity+age.cat, design=nhanes.ao.design) %>% 
  as.data.frame() %>% 
  group_by(abd_obesity) %>% 
  mutate(prop.ao = Freq/sum(Freq))
...
#for table 1
svytable(~abd_obesity+race, design=nhanes.ao.design) %>% 
  as.data.frame() %>% 
  group_by(abd_obesity) %>% 
  mutate(prop.ao = Freq/sum(Freq))
...
#for table 1
svytable(~abd_obesity+obesity_cat, design=nhanes.ao.design) %>% 
  as.data.frame() %>% 
  group_by(abd_obesity) %>% 
  mutate(prop.ao = Freq/sum(Freq))
